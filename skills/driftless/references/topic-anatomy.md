...
- Whether `context load --files <file>` matches the topic to a file.
- Whether a push to the file marks the topic stale.
- Whether the topic shows up in `graph coverage` as covering a component.

### Healthy anchor count

| Component count covered | Verdict |
|---|---|
...
| `suggested` | Auto-generated by `driftless init --suggest` |
